...
export default function ExploreCampaignsPage() {
  ...
  }, [searchQuery, selectedCategory, selectedStatus, selectedPledge, campaigns]);
      ...
      <main className="flex-1 pt-24 pb-20 w-full px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="bg-white p-2 md:p-3 rounded-lg shadow-[0_2px_15px_rgb(0,0,0,0.04)] border border-gray-200 mb-12 flex flex-col md:flex-row items-center gap-3 w-full">
          ...
          <div className="flex w-full md:w-auto items-center gap-3 flex-wrap md:flex-nowrap px-2">
            <button className="flex-1 md:flex-none flex justify-between items-center bg-gray-50 border border-gray-200 px-4 py-2 rounded-md text-[13px] font-semibold text-gray-700 hover:bg-gray-100 transition-colors">
              All Categories <ChevronDown className="w-3.5 h-3.5 ml-2 text-gray-500" />
            </button>
            <button className="flex-1 md:flex-none flex justify-between items-center bg-gray-50 border border-gray-200 px-4 py-2 rounded-md text-[13px] font-semibold text-gray-700 hover:bg-gray-100 transition-colors">
              Any Status <ChevronDown className="w-3.5 h-3.5 ml-2 text-gray-500" />
            </button>
            <button className="flex-1 md:flex-none flex justify-between items-center bg-gray-50 border border-gray-200 px-4 py-2 rounded-md text-[13px] font-semibold text-gray-700 hover:bg-gray-100 transition-colors">
              Sort By: Trending <ChevronDown className="w-3.5 h-3.5 ml-2 text-gray-500" />
            </button>
            <button className="flex-none flex items-center border border-indigo-200 bg-indigo-50/50 text-indigo-600 px-4 py-2 rounded-md text-[13px] font-bold hover:bg-indigo-50 transition-colors">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <aside className="w-full lg:w-64 flex-shrink-0">
            <div className="bg-white rounded-xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 p-6 sticky top-24">
              
              {/* Categories Filter */}
              <div className="mb-8">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-[14px] font-bold text-gray-900">Categories</h3>
                  {selectedCategory !== 'All' && (
                    <button onClick={() => setSelectedCategory('All')} className="text-[11px] text-indigo-600 font-semibold hover:underline">Clear</button>
                  )}
                </div>
                <div className="space-y-3">
                  {['Technology', 'Art', 'Community', 'Design', 'Film', 'Games', 'Food'].map((cat) => (
                    <label key={cat} className="flex items-center cursor-pointer group" onClick={() => setSelectedCategory(cat)}>
                      <div className={`w-4 h-4 rounded border flex items-center justify-center mr-3 transition-colors ${selectedCategory === cat ? 'bg-[#12643E] border-[#12643E]' : 'border-gray-300 group-hover:border-[#12643E]'}`}>
                        {selectedCategory === cat && <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg>}
                      </div>
                      <span className={`text-[14px] transition-colors ${selectedCategory === cat ? 'text-gray-900 font-bold' : 'text-gray-600 group-hover:text-gray-900'}`}>{cat}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="mb-8">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-[14px] font-bold text-gray-900">Funding Status</h3>
                  {selectedStatus !== 'All' && (
                    <button onClick={() => setSelectedStatus('All')} className="text-[11px] text-indigo-600 font-semibold hover:underline">Clear</button>
                  )}
                </div>
                <div className="space-y-3">
                  {['All', 'Active', 'Ending Soon', 'Fully Funded'].map((status) => (
                    <label key={status} className="flex items-center cursor-pointer group" onClick={() => setSelectedStatus(status)}>
                      <div className={`relative w-4 h-4 rounded-full border mr-3 flex items-center justify-center transition-colors ${selectedStatus === status ? 'border-[#12643E]' : 'border-gray-300 group-hover:border-[#12643E]'}`}>
                        {selectedStatus === status && <div className="w-2 h-2 rounded-full bg-[#12643E]"></div>}
                      </div>
                      <span className={`text-[14px] transition-colors ${selectedStatus === status ? 'text-gray-900 font-bold' : 'text-gray-600 group-hover:text-gray-900'}`}>{status}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-[14px] font-bold text-gray-900">Pledge Amount</h3>
                  {selectedPledge !== 'All' && (
                    <button onClick={() => setSelectedPledge('All')} className="text-[11px] text-indigo-600 font-semibold hover:underline">Clear</button>
                  )}
                </div>
                <div className="space-y-3">
                  {['All', 'Under $50', '$50 - $100', '$100 - $500', '$500+'].map((amount) => (
                    <label key={amount} className="flex items-center cursor-pointer group" onClick={() => setSelectedPledge(amount)}>
                      <div className={`relative w-4 h-4 rounded-full border mr-3 flex items-center justify-center transition-colors ${selectedPledge === amount ? 'border-[#12643E]' : 'border-gray-300 group-hover:border-[#12643E]'}`}>
                        {selectedPledge === amount && <div className="w-2 h-2 rounded-full bg-[#12643E]"></div>}
                      </div>
                      <span className={`text-[14px] transition-colors ${selectedPledge === amount ? 'text-gray-900 font-bold' : 'text-gray-600 group-hover:text-gray-900'}`}>{amount}</span>
                    </label>
                  ))}
                </div>
              </div>
